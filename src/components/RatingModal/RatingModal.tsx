...
type Props = {
  ...
  reviewHandleSubmit: () => Promise<string | undefined>;
    ...
    >
      <div className="bg-white w-96 p-4 rounded-lg shadow-lg">
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center">
            {startValues.map((value) => (
              <button
                className={`w-6 h-6 ${ratingValue === value ? "text-yellow-500" : "text-gray-400"}`}
        ...
        <div className="flex justify-end">
          <button
            onClick={reviewHandleSubmit}
            disabled={submittingReview}
            className="px-4 py-2 text-white bg-primary rounded-md mr-2 hover:opacity-70"
          >
            {submittingReview ? "Submitting..." : "Submit"}
          </button>
          <button
            onClick={toggleRatingModal}
            className="px-4 py-2 ml-2 text-gray-700 bg-gray-300 rounded-md hover:bg-gray-400"
          >
            Cancel
          </button>
        </div>
